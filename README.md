...
- `app/public/btc_indicators_history_light.json`: legacy lightweight export generated by the pipeline, no longer used by the frontend
...
3. Script archives current JSON outputs before overwrite, then writes full history, latest snapshot, manifest, event files, and a legacy light export.
