...
```bash
# API 配置
BTC_UPDATE_INTERVAL=600          # 更新间隔（秒）
BTC_UPDATE_TIME=08:00,20:00       # 每日更新时间
BTC_LOG_LEVEL=INFO               # 日志级别

# 可选 API 密钥
COINGECKO_DEMO_API_KEY=your_key  # CoinGecko API 密钥（可选）
```
...
1. **Fork 项目** 并创建特性分支
2. **遵循代码规范**（Python 使用 PEP 8，JavaScript 使用 ESLint）
3. **添加测试** 确保功能正常
4. **提交 PR** 并描述变更内容
...
2. **数据文件权限错误**
   ```bash
   # 修复权限
   chmod 644 data/latest/btc_indicators_latest.json
   chmod 644 data/history/btc_indicators_history.json
   ```
